 ...
 *
 */
class CRM_Report_Form_Contribute_SoftCreditSummary extends CRM_Report_Form {
  ...
  public $_drilldownReport = array('contribute/scdetail' => 'Link to Detail Report');
  ...
  function alterDisplay(&$rows) {
    ...
    foreach ($rows as $rowNum => $row) {
      ...
      if (!empty($this->_params['group_bys']['receive_date']) && !empty($row['civicrm_contribution_receive_date_start']) &&
        ...
        $endDate   = CRM_Utils_Date::customFormat($row['civicrm_contribution_receive_date_start'], '%Y:%m:%d');
        $dateEnd   = array();

        list($dateEnd['Y'], $dateEnd['M'], $dateEnd['d']) = explode(':', $endDate);
        ...
        $url = CRM_Report_Utils_Report::getNextUrl('contribute/scdetail',
      ...
      $entryFound = $this->alterDisplayAddressFields($row, $rows, $rowNum, 'contribute/scdetail', 'List all contribution(s) for this ') ? TRUE : $entryFound;
